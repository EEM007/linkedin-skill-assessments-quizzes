...
- [X] No. You cannot combine or merge two list entries.
...
- [X] You do not want to print the bill.
...
- [X] Wait until you do the bank reconciliation and enter one journal entry for all of your debit card purchases.
...
- [X] Payroll Liabilities and Payroll Expenses
...
- [X] Credit Checking and debit Undeposited Funds.
...
- [X] Condense the data file on the old computer to the drive space. Back up the file and then restore the condensed data file to the new computer.
...
- [X] Click Company > Prepare Letters with Envelopes > Customize Letter Templates.
...
- [X] Someone changed or deleted a transaction you cleared on the prior reconciliation.
...
- [X] Click the Automate button when the transaction is on the screen.
...
- [X] QuickBooks needs this information to automatically make the payments for you on or before the due date.
...
- [X] to flag when you need more staff for a project
...
- [X] Press the F2 key (Windows) or select Help > Product Information (Mac) to display the version information.
...
- [X] Click the account to select it, and then click Lists > Chart of Accounts > Make subaccount.
...
- [X] Credit Cash and debit Accounts Payable.
...
- [X] Create a group that contains all of the components of the widget.
...
- [X] Subtracts net profit from an Owner's or Shareholder's Equity account.
...
- [X] Manually change the Amount field to the discounted total for the item.
...
- [X] Sales Taxes, Inventory, and Progress Invoicing
...
- [X] You are required to use a more complex password.
...
- [X] Edit the user's access to restrict their ability to work with transactions.
...
- [X] Employee Balance Detail would be affected.
...
- [X] You can click and drag a diamond to move the list entry to a different location on the list.
